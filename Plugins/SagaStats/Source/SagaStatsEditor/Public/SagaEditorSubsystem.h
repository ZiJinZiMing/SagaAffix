...
#include "SagaEditorSubsystem.generated.h"

class FTokenizedMessage;
class ISSAttributeReferencerHandler;
class IMessageToken;
class IToolkit;
class UEdGraphPin;
 ...
 */
UCLASS()
class SAGASTATSEDITOR_API USagaEditorSubsystem : public UEditorSubsystem
{
	GENERATED_BODY()
...
public:
	...
	static USagaEditorSubsystem& Get();
	...
	virtual void RegisterReferencerHandler(const FName& InClassName, const TSharedPtr<ISSAttributeReferencerHandler>& InHandler);
	...
	TSharedPtr<ISSAttributeReferencerHandler> FindAssetDependencyHandler(const FName& InReferencer, TWeakObjectPtr<UObject>& OutDefaultObject);
